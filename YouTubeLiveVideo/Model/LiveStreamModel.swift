...
// The snippet object contains basic details about the stream, including its channel, title, and description.
   ...
   public struct Snipped {
      let publishedAt: String    // The date and time that the stream was created. The value is specified in ISO 8601 (YYYY-MM-DDThh:mm:ss.sZ) format.
      let channelId: String      // The ID that YouTube uses to uniquely identify the channel that is transmitting the stream.
      let title: String          // The stream's title. The value must be between 1 and 128 characters long.
      let description: String    // The stream's description. The value cannot be longer than 10000 characters.
      let isDefaultStream: Bool  // Indicates whether this stream is the default stream for the channel. ()
   }

   public struct IngestionInfo {
      let streamName: String
      let ingestionAddress: String  // cdn.ingestionInfo.ingestionAddress	string
...
// The cdn object defines the live stream's content delivery network (CDN) settings. These settings provide details about the manner in which you stream your content to YouTube.
   ...
   public struct CDN {
      let format: String  // This property has been deprecated as of April 18, 2016. Instead, please use the cdn.frameRate and cdn.resolution properties to specify the frame rate and resolution separately.
      let ingestionType: String     // The method or protocol used to transmit the video stream. (dash or rtmp)
      let ingestionInfo: IngestionInfo    // The ingestionInfo object contains information that YouTube provides that you need to transmit your stream to YouTube.
      let resolution: String     // The resolution of the inbound video data.
      let frameRate: String      // The frame rate of the inbound video data.
   }
   ...
   public struct Status {
      let streamStatus: String // Valid values for this property are:
...
//      ready – The stream has valid CDN settings.
      let healthStatus: HealthStatus   // This object contains information about the live stream's health status, which could be used to identify, diagnose, and resolve streaming problems.
   }
   
   let kind: String        // Identifies the API resource's type. The value will be youtube#liveStream.
   let etag: String        // The Etag of this resource.
   let id: String          // The ID that YouTube assigns to uniquely identify the stream.
   let snipped: Snipped
   let cdn: CDN
   let status: Status
}
